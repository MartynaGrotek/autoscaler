...
type NodeGroupChangeObserver interface {
	...
	RegisterFailedScaleUp(nodeGroup cloudprovider.NodeGroup, delta int, errorInfo cloudprovider.InstanceErrorInfo, gpuResourceName, gpuType string, draDriverNames string, currentTime time.Time)
...
func (l *NodeGroupChangeObserversList) RegisterFailedScaleUp(nodeGroup cloudprovider.NodeGroup, delta int, errorInfo cloudprovider.InstanceErrorInfo, gpuResourceName, gpuType, draDriverNames string, currentTime time.Time) {
	l.mutex.Lock()
	defer l.mutex.Unlock()
	for _, observer := range l.observers {
		observer.RegisterFailedScaleUp(nodeGroup, delta, errorInfo, gpuResourceName, gpuType, draDriverNames, currentTime)
	}
}
